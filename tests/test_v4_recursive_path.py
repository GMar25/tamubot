...
def test_recursive_retrieval_calls_hybrid_search_per_course():
    ...
    with patch("tamubot.rag.tools.mongo.hybrid_search", return_value=chunks) as mock_hs, \
...
def test_recursive_retrieval_writes_recursive_chunks_not_retrieved():
    ...
    with patch("tamubot.rag.tools.mongo.hybrid_search", return_value=[]), \
         patch("tamubot.rag.tools.voyage.rerank", side_effect=lambda q, c, top_k: c):
        result = recursive_retrieval_node(state)
...
def test_recursive_retrieval_handles_multiple_courses():
    ...
    with patch("tamubot.rag.tools.mongo.hybrid_search", side_effect=[chunk_a, chunk_b]), \
         patch("tamubot.rag.tools.voyage.rerank", side_effect=lambda q, c, top_k: c):
        result = recursive_retrieval_node(state)
...
def test_recursive_retrieval_fallback_on_error():
    ...
    with patch("tamubot.rag.tools.mongo.hybrid_search", side_effect=Exception("DB down")):
        result = recursive_retrieval_node(state)
...
def test_recursive_router_overwrites_function_and_query():
    ...
    with patch("tamubot.rag.tools.llm.call_llm") as mock_llm:
...
def test_recursive_router_fallback_on_llm_failure():
    ...
    with patch("tamubot.rag.tools.llm.call_llm", side_effect=Exception("LLM timeout")):
        result = recursive_router_node(state)
...
def test_generator_node_uses_recursive_prompt_when_recursive_search_true():
    ...
    with patch("tamubot.rag.generator.generate_stream", side_effect=fake_generate_stream):
...
def test_route_after_retrieval_does_not_exist():
    import tamubot.rag.edges.routing as routing_mod
